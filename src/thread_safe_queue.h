...
{
  public:
    // Returns whether could push/pop or queue was closed.  Currently, we only implemented the
    // blocking operations.
    ...
    void push(T const& v)
    {
        ...
        // Pushing with lock, only using list<>::splice().
        {
            ...
            while (currentSize == maxSize) cvPush.wait(lock);
    ...
    QueueResult pop(T& v)
    {
        ...
        // Pop into tmpList which is finally written out.
        {
            ...
            while (list.empty() && state != State::CLOSED) cvPop.wait(lock);

            // If the queue was closed and the list is empty then we cannot return anything.
            if (list.empty() && state == State::CLOSED) return CLOSED;
